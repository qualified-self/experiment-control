...
class EdfFile(object):
    ...
    def __init__(self,fn):
        ...
        efile = self.efile
        self.duration_sec = efile.file_duration
        nsignals = efile.signals_in_file
        self.nsignals=nsignals
        nsamples = np.array([efile.samples_in_file(chn) for chn in xrange(nsignals)])
        self.nsamples = nsamples
        sample_freqs = np.array([efile.samplefrequency(chn) for chn in xrange(nsignals)])
        self.signal_sample_freqs = sample_freqs
        signal_text_labels = [efile.signal_label(chn).strip() for chn in xrange(nsignals)]
        self.signal_text_labels =signal_text_labels # note the original EDF file may need the extra spaces
...
def edf2hdf5(fn):
    ...
    nsamples = [ef.samples_in_file(ii) for ii in xrange(nsigs)]
    nsample0 = nsamples[0]
    nsamples = np.array(nsamples)

    if any(nsamples != nsample0):
        raise "Assumption error: should be equal rate or shoudl all something else"
...
if __name__ == '__main__':
    ...
    for chn in xrange(f.nsignals):
        print "reading signal %d" % chn, 
        v = data[chn*L:(chn+1)*L]
        f.efile.readsignal(chn, 0, L,v)
    ...
    sf = h5.createArray(h5.root, 'signal_sample_freqs', f.signal_sample_freqs,'sampling frequency for each signal')
